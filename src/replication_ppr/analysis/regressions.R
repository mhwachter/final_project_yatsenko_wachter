...
ctrls = c("ldist", "lrgdp", "lrgdppc", "comlang", "border", "landl", "island",
          "lareap", "comcol", "curcol", "colony", "comctry", "custrict",
          "regional", "bothin", "onein", "gsp")
ctrls_tab8 = c("ldist", "lrgdp", "lrgdppc", "comlang", "border", "landl", "island",
               "lareap", "comcol", "curcol", "colony", "comctry", "custrict",
               "regional", "gsp")
ctrls_tab3 <- c("ldist", "lrgdp", "lrgdppc", "comlang", "border", "landl", "island",
                "lareap", "comcol", "curcol", "colony", "comctry", "custrict",
                "regional", "gsp")
...
tab1 <- modelsummary(tab1,fmt = 2, vcov = ~pairid,
                     coef_map = c("bothin","onein","gsp","ldist","lrgdp","lrgdppc","regional",
...
tab2 <- feols(fml = ltrade ~ .[ctrls] | year, data = df,
              split = ~year %keep% c("1950", "1955","1960", "1965","1970", "1975","1980",
                                     "1985", "1990", "1995"))
tab2 <- modelsummary(tab2, fmt = 2, vcov = ~pairid, gof_map = "none",
             coef_map = c("bothin","onein","gsp"), group = model ~ term,
             output = "latex",title = "Cross-Sectional Analysis")

# Table 3 -----------------------------------------------------------------

df$periods = cut(df$year,breaks = c(1948,1949,1951,1956,1961,1967,1979,1994,2000),
                 labels = c("Before Annecy round (1949)",
                            ...
                            "After Uruguay round"),include.lowest = TRUE, right = FALSE)
tab3 <- list(
...
tab3 <- modelsummary(tab3, fmt = 2, vcov = ~pairid, gof_map = "none",
...
tab4 <- list(
  "Default" = feols(fml = ltrade ~ .[ctrls] | year, data = df),
  "South Asia" = feols(fml = ltrade ~ .[ctrls] | year,
                       data = df[sasia1 == 1 | sasia2 == 1]),
  "East Asia" = feols(fml = ltrade ~ .[ctrls] | year,
                      data = df[easia1 == 1 | easia2 == 1]),
  "Sub-Saharan Africa" = feols(fml = ltrade ~ .[ctrls] | year,
                               data = df[ssafr1 == 1 | ssafr2 == 1]),
  "Middle East or North Africa" = feols(fml = ltrade ~ .[ctrls] | year,
                                        data = df[menaf1 == 1 | menaf2 == 1]),
  "Latin America or Caribbean" = feols(fml = ltrade ~ .[ctrls] | year,
                                       data = df[latca1 == 1 | latca2 == 1]),
  "High income" = feols(fml = ltrade ~ .[ctrls] | year,
                        data = df[highi1 == 1 | highi2 == 1]),
  "Middle income" = feols(fml = ltrade ~ .[ctrls] | year,
                          data = df[midin1 == 1 | midin2 == 1]),
  "Low income" = feols(fml = ltrade ~ .[ctrls] | year,
                       data = df[lowin1 == 1 | lowin2 == 1]),
  "Least developed" = feols(fml = ltrade ~ .[ctrls] | year,
                            data = df[least1 == 1 | least2 == 1])
)
tab4 <- modelsummary(tab4, fmt = 2, vcov = ~pairid, gof_map = "none",
             coef_map = c("bothin","onein","gsp"), group = model ~ term,
...
tab5 <- list(
  ...
  "Only industrial countries" = feols(fml = ltrade ~ .[ctrls] | year,
                                      data = df[cty1 < 200 & cty2 < 200]),
  "No African countries" = feols(fml = ltrade ~ .[ctrls] | year,
                                 data = df[ssafr1 == 0 & ssafr2 == 0 & menaf1 == 0 & menaf2 == 0]),
  "No Latin or Caribbean countries"  = feols(fml = ltrade ~ .[ctrls] | year,
                                             data = df[latca1 == 0 & latca2 == 0]),
  "No OPEC countries" = feols(fml = ltrade ~ .[ctrls] | year,
                             data = df[!(cty1 %in% opec) & !(cty2 %in% opec)]),
  "No RTA observations" = feols(fml = ltrade ~ .[ctrls] | year,
                               data = df[regional == 0]),
  "Without poorest quartile of real GDP p/c" = feols(fml = ltrade ~ .[ctrls] | year,
                                                     data = df[quartile_lrgdppc != 1]),
  "Without smallest quartile of real GDP" = feols(fml = ltrade ~ .[ctrls] | year,
                                                data = df[quartile_lrgdp != 1]),
  "Without 3sd outliers" = feols(fml = ltrade ~ .[ctrls] | year,
                               data = df[outliers != 1]),
  "Only Canadian observations" = feols(fml = ltrade ~ .[ctrls] | year,
                                      data = df[cty1 == 156 | cty2 == 156]),
  "Only American observations" = feols(fml = ltrade ~ .[ctrls] | year,
                                       data = df[cty1 == 111 | cty2 == 111]),
  "Only British observations" = feols(fml = ltrade ~ .[ctrls] | year,
                                      data = df[cty1 == 112 | cty2 == 112]),
  "Only French observations" = feols(fml = ltrade ~ .[ctrls] | year,
                                     data = df[cty1 == 132 | cty2 == 132]),
  "Only Italian observations" = feols(fml = ltrade ~ .[ctrls] | year,
                                      data = df[cty1 == 136 | cty2 == 136]),
  "Only German observations" = feols(fml = ltrade ~ .[ctrls] | year,
                                     data = df[cty1 == 134 | cty2 == 134]),
  "Only Japanese observations" = feols(fml = ltrade ~ .[ctrls] | year,
                                       data = df[cty1 == 158 | cty2 == 158])
)
tab5 <- modelsummary(tab5, fmt = 2, vcov = ~pairid, gof_map = "none",
                     coef_map = c("bothin","onein","gsp"), group = model ~ term,
...
tab8 <- modelsummary(tab8,fmt = 2, vcov = ~pairid, gof_map = c("r.squared", "rmse"),
             coef_map = c("bothin","onein","gsp","found1","found2","minyrs",
                          "maxyrs","ldist","lrgdp","lrgdppc","regional","custrict",
                          "comlang","border","landlocked","island","lareap","comcol",
                          "curcol","colony","comctry"),output = "latex",
